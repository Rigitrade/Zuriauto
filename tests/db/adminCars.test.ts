...
function cookie(): Record<string, string> {
  return { cookie: `${ADMIN_COOKIE}=${issueAdminSession()}` };
}

function addRequest(body: unknown, signed = true): Request {
  return new Request("https://zuriauto.ch/api/admin/cars/", {
    method: "POST",
    headers: { "content-type": "application/json", ...(signed ? cookie() : {}) },
    body: JSON.stringify(body),
  });
}

function patchRequest(body: unknown, signed = true): Request {
...
describe("POST /api/admin/cars", () => {
  ...
  it("refuses an unfenced request", async () => {
    const response = await POST(
      addRequest({ model: "Toyota Prius", plate: "ZH 1" }, false)
    );
  ...
  it("adds a car with a derived slug, available", async () => {
    await org();
    const response = await POST(
      addRequest({ model: "Toyota Prius Hybrid", plate: "zh 777 111" })
    );
  ...
  it("makes the new car offerable at the desk immediately", async () => {
    await org();
    await POST(addRequest({ model: "VW Golf", plate: "ZH 777 222" }));
  ...
  it("answers 409 for a plate already on the fleet", async () => {
    await org();
    await POST(addRequest({ model: "Toyota Prius", plate: "ZH 777 333" }));
    const again = await POST(
      addRequest({ model: "Something Else", plate: "ZH 777 333" })
    );
  ...
  it("rejects a car with no plate", async () => {
    await org();
    const response = await POST(addRequest({ model: "Toyota Prius", plate: "" }));
...
describe("PATCH /api/admin/cars/[id]", () => {
  ...
  it("refuses an unfenced request", async () => {
    ...
    const response = await PATCH(
      patchRequest({ model: "Hacked" }, false),
      params(car.id)
    );
    expect(response.status).toBe(401);
  });
  ...
  it("edits the plate without changing the slug", async () => {
    ...
    const response = await PATCH(
      patchRequest({ plate: "ZH 999 888", vin: "JTDKB20U" }),
      params(before.id)
    );
  ...
  it("takes a car off the road and puts it back", async () => {
    ...
    await PATCH(patchRequest({ status: "retired" }), params(car.id));
    expect(await fleetSlugs()).not.toContain("prius-zh513925");

    await PATCH(patchRequest({ status: "available" }), params(car.id));
  ...
  it("rejects an empty update", async () => {
    ...
    await seedFleet(prisma, id);
    const car = await prisma.car.findFirstOrThrow();
    const response = await PATCH(patchRequest({}), params(car.id));
    expect(response.status).toBe(400);
  });
});
